...
from rp2.localization import _ as LocalLanguageText
from rp2.logger import create_logger
from rp2.plugin.report.abstract_ods_generator import AbstractODSGenerator
from rp2.rp2_decimal import ZERO, RP2Decimal
from rp2.rp2_error import RP2TypeError

LOGGER: logging.Logger = create_logger("tax_report_be")
...
class Generator(AbstractODSGenerator):
    ...
    def generate(
        ...
        for asset, computed_data in asset_to_computed_data.items():
            ...
            self.__generate(output_file, asset, computed_data.gain_loss_set, row_indexes, total_gains)
        ...
        for sheet_name in output_file.sheets.names():
            if (sheet_name not in ("Legend", "Légende")) and row_indexes[sheet_name] == Generator.HEADER_ROWS:
        ...
        # Adds a summary of total gains at the end of the "Legend" sheet
        legend_sheet: Any | None = next((sheet for sheet in output_file.sheets if sheet.name == LocalLanguageText("Legend")), None)
        if legend_sheet:
            row_index: int = 25
            for sheet_type, total_gain in total_gains.items():
                self._fill_cell(legend_sheet, row_index, 0, f"Total {sheet_type}", visual_style="transparent_border")
                self._fill_cell(legend_sheet, row_index, 1, total_gain, visual_style="transparent_border", data_style="fiat")
                row_index += 1
    ...
    def __generate(self, output_file: Any, asset: str, gain_loss_set: GainLossSet, row_indexes: Dict[str, int], total_gains: dict[str, RP2Decimal]) -> None:
        ...
        for sheet in output_file.sheets:
            if sheet.name == LocalLanguageText("Legend"):
                continue
        ...
        for entry in gain_loss_set:
            ...
            total_gains[_TYPE_TO_SHEET[sheet_type]] += gain_loss.fiat_gain
